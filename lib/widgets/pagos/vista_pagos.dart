...
class VistaPagos extends StatefulWidget {
  final String hijoID;
  const VistaPagos({required this.hijoID});
    ...
    return Scaffold(
      ...
      body: cargando
          ...
          : pagos.isEmpty
              ...
              : ListView.builder(
                  padding: EdgeInsets.fromLTRB(16, 16, 16, bottomPadding), // margen inferior extra
                  itemCount: pagos.length,
                  itemBuilder: (context, index) {
                    final pago = pagos[index];
                    final nombreArchivo = pago.nombreArchivo ?? pago.urlArchivo?.split('/').last;

                    final sumaTotal = pago.aportaciones.values.fold(0.0, (a, b) => a + b);
                    ...
                    final resumenAportaciones = pago.aportaciones.entries
                        .map((e) => '👤 ${e.key.substring(0, 6)}: €${e.value.toStringAsFixed(2)}')
                        .join(' · ');

                    final esCompartido = config?.gastosCompartidos.any((g) =>
                        pago.titulo.toLowerCase().contains(g.toLowerCase().trim())) ?? false;
                        ...
                        child: Column(
                          ...
                          children: [
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      if (config?.notasPersonalizadas.isNotEmpty ?? false)
                                        Text(
                            ...
                            Text(
                              ...
                              style: TextStyle(
                                color: estadoCalculado == 'Pagado' ? Colors.green : Colors.orange,
                                fontWeight: FontWeight.bold,
                                fontFamily: 'Montserrat',
                              ),
                            ),
                            ...
                            if (nombreArchivo != null && pago.urlArchivo != null) ...[
                              const SizedBox(height: 8),
                              TextButton.icon(
                                icon: const Icon(Icons.visibility),
                                label: const Text('Ver justificante',
                                    style: TextStyle(fontFamily: 'Montserrat')),
                                onPressed: () => DocumentoHelper.ver(context, nombreArchivo, pago.urlArchivo!),
                              ),
                              TextButton.icon(
                                icon: const Icon(Icons.download),
                                label: const Text('Descargar justificante',
                                    style: TextStyle(fontFamily: 'Montserrat')),
                                onPressed: () => DocumentoHelper.descargar(context, nombreArchivo, pago.urlArchivo!),
                              ),
                            ],
                            if (pago.estado == 'En disputa' && pago.motivoEstado.trim().isNotEmpty)
                              Padding(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () => formularioPago(context: context, hijoID: widget.hijoID, onGuardado: cargarDatos),
